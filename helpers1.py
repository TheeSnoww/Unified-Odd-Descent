...
def input_optimization_ncopt(Ws_, bs_,
    ...
    for counter in range(T):
        ...
        for W, b, diag_sigma in zip(Ws[:-1], bs[:-1], diag_sigma_list):
            ...
            x_constraint_loss1 = targets  * 4 * (torch.nn.functional.sigmoid(prob - 0.5) - 0.5).cuda()
            x_constraint_loss2 = 0
...
#             assert not torch.any(torch.isnan(diag_sigma_activations))
        ...
        torch.ones(prod_W_norm.shape).cuda())
            ...
            prob = diag_sigma
            start_time = time.time()  # 记录每次迭代开始的时间


            constraint_loss1 = (targets  * 4 * (torch.nn.functional.sigmoid(prob - 0.5) - 0.5).cuda()).max()
            constraint_loss += constraint_loss1
            
            # constraint_loss1 = torch.maximum(targets  * 4 * (torch.nn.functional.sigmoid(prob - 0.5) - 0.5).cuda(),
            #                                  torch.zeros(diag_sigma.shape).cuda()).max()
            # constraint_loss += constraint_loss1

            # constraint_loss1 = torch.maximum(targets  * torch.nn.functional.softplus(prob - 0.5).cuda(),
            #                                  torch.zeros(diag_sigma.shape).cuda()).max()
            # constraint_loss2 = torch.maximum((- targets ) * torch.nn.functional.softplus(0.5 - prob).cuda(),
            #                                  torch.zeros(diag_sigma.shape).cuda()).max()
...
#         assert torch.all(torch.isfinite(constraint_loss))
        ...
        for counter_sigma in range(len(diag_sigma_list)):
            ...
            if not torch.isclose(grad.norm(), torch.zeros(grad.norm().shape).cuda(),
                     atol = 1e-4) and grad.norm() > 1.:
                grad /= grad.norm()
            if perturb_freq is not None and (counter - last_perturb_iter[counter_sigma]) >= perturb_freq:
        ...
        end_time = time.time()  # 记录每次迭代结束的时间
        iteration_times.append(end_time - start_time)  # 计算并存储该迭代的运行时间
